...
# settings
res = width, height = 800, 450 # '1600, 900'
...
class Fractal:
    def __init__(self, app):
        self.app = app
        self.screen_array = np.full((width, height, 3), [0, 0, 0], dtype=np.uint32) # Kolory w formacie RGB.

        ti.init(arch=ti.cpu) # ti.cpu, ti.cuda, ti.opengl, ti.vulkan, ti.metal


        # Pola są tworzone w celu przechowywania kolorów i tekstur
    ...
    # Metoda delta_time służy do obliczania czasu, jaki upłynął między aktualizacjami fraktala
    def delta_time(self):
    ...
    # Metoda render przedstawia podstawową logikę generowania fraktala Mandelbrota
    @ti.kernel
    def render(self, max_iter: ti.int32, zoom: ti.float32, dx: ti.float32, dy: ti.float32):

        for x, y in self.screen_field:

            # Dla każdego piksela określana jest liczba zespolona c, która zależy od bieżących współrzędnych piksela (x i y), powiększenia i przesunięcia (dx i dy)
            c = ti.Vector([(x - offset[0]) * zoom - dx, (y - offset[1]) * zoom - dy])
            z = ti.Vector([0.0, 0.0])
            num_iter = 0

            # Aktualizuje wartość 'z' w każdej iteracji pętli.
            ...
            # Definiowana jest wartość 'col', która jest używana do wyboru koloru piksela z self.texture_field
            col = int(texture_size * num_iter / max_iter)

            # Kolor piksela z 'self.texture_field' jest przypisywany do odpowiadającego mu piksela w 'self.screen_field'
            self.screen_field[x, y] = self.texture_field[col, col]


    # W tej metodzie odbywa się przetwarzanie kontrolne w celu dostosowania parametrów fraktala.
    def control(self):
    ...
    # Metoda update() służy do aktualizacji stanu fraktala
    def update(self):
        self.control()
    ...
    # Metoda draw() służy do renderowania fraktala na ekranie
    def draw(self):
